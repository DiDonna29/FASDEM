...
public class PerTipoProveedor extends PerGeneric {

	private static final String T_TABLA = "TipoProveedor.";
	private SqlMapClient cnx = null;

	public PerTipoProveedor() {
	...
	public List listByTipoTramiteR(int id) throws PersonalNotFoundException, SQLException {
		List list = new ArrayList();
		list = cnx.queryForList(T_TABLA.concat("listByTipoTramiteRmbls"), id);
		if (list.size() != 0)
			return list;
		else
			throw new PersonalNotFoundException(" ");
