				...
				<table class="tabla" width="600" cellpadding="2" cellspacing="1"
					...
					<tr class="tituloCabecera">
						...
						<td><bean:message key="general.causaingreso"
							bundle="etiquetas" /></td>
						<td><bean:message key="general.observacion"
							bundle="etiquetas" /></td>
						<div class="s10"></div>
					</tr>
					<tr class="item" bgcolor="white">
						<td class="nblack"> <c:out value="${Op.aniomesCodigo}" />-<c:out
						...
						<td><c:out value="${Op.observacion}" /></td>
					<tr class="item" bgcolor="white"><c:if test="${Op.estatus.id==9}">
						<td align="center"><img
							...
							onclick="javascript:openWinScroll2('<%=request.getContextPath()%>/security/comunes/generareporte.do?id=<c:out
								value="${Op.id}" />&anio=<c:out
								value="${Op.anioSiniestro}" />&id_reporte=1','OrdenDeMedicinas',800,600);">



						</td></c:if></tr>

				</table><c:if test="${Op.estatus.id!=9}"><table class="tabla" width="700" cellpadding="2" cellspacing="1"
					border="0">
					<tr class="tituloCabecera"><td align="center">
					 El reporte de Orden de Medicinas solo se puede imprimir cuando su estatus es Egresado.
					</td></tr></table></c:if>
				</div>
