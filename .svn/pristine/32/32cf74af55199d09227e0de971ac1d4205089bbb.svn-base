...
<%
		...
		Clinica proveedor = (Clinica)request.getAttribute("proveedor");
		Cuenta cuenta = proveedor.getObjcuenta();
...
<div class="sp5"></div>
				...
				<%}else{%>
				<input class="boton" value="Modificar" type="button"  onclick="modificaP('<%=proveedor.getId()%>');" />
				...
				<input id="crea" type="hidden" name="crea" value="<%=proveedor.getId()%>">
...
<script language="JavaScript">
	function ir(acc){
		...
		document.forms[0].action = "<%=request.getContextPath()%>/security/administradores/CreaCuentaProveedor.do"
