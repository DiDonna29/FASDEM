...
<tiles:insert page="/jsp/plantilla/plantilla.jsp">
	...
	<tiles:put name="cuerpo" direct="true">
		...
		<input type="hidden" name="anioSiniestro" id="anioSiniestro"/>
		...
		<c:if test="${resultado!=null}">
			...
			<table class="tabla" cellpadding="2" cellspacing="1" border="0" bgcolor="yellow">
				...
				<pg:pager maxPageItems="<%=20%>" maxIndexPages="10" url="consulta.do" isOffset="false" export="offset,currentPageNumber=pageNumber" scope="request">
					<pg:param name="cedula" />
					<pg:param name="nombres" />
					<pg:param name="codigo" />
					<c:forEach items="${resultado}" var="sini">
						<pg:item>
							<tr class="item" bgcolor="white" onclick="javaScript:submit2('<c:out value="${sini.id}" />','<c:out value="${sini.anioSiniestro}" />','<c:out value="${sini.estatus.id}" />')">
								...
								<td><c:out value="${sini.tipoEmpleado.descripcion}" /></td>
...
<script>
function submit2(argid, arganio,es){
	if (es=="4"){
	 alert ("Imposible cambiar el Tipo de Empleado (Siniestro Liquidado)");
	}else{
	document.getElementById("idSini").value=argid;
	document.getElementById("anioSiniestro").value=arganio;
	document.forms[0].action='<%=request.getContextPath()%>/security/modificaTipoEmpleado/detalle.do';
