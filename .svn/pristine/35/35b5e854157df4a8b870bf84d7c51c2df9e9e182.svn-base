...
<div class="etiqueta titulo cgp"><bean:message
		...
		<table class="tabla" width="700" cellpadding="2" cellspacing="1"
			...
			<tr class="item" bgcolor="white">
					<td>
					<div class="etiqueta titulo cgp">Liquidación</div>
					<img src="<%=request.getContextPath()%>/images/printer.png"
						onclick="javascript:openWinScroll2('<%=request.getContextPath()%>/security/comunes/generareporte.do?id=<c:out
								value="${siniestro.id}" />&anio=<c:out
		...
		<div class="parametro titulo "><fmt:formatNumber
						value="${siniestro.montoPresupuestado - siniestro.montoAmparado}" pattern='###0.00' /> Bs.</div>
...
<c:if test="${factura.numeroFactura!=null}">
		...
		<table class="tabla" width="500" cellpadding="1" cellspacing="1" border="0" bgcolor="yellow">
			...
			<c:forEach items="${listAutorizacion}" var="f">
				
				<tr class="item" bgcolor="white">
					
					<td><fmt:formatDate pattern="dd/MM/yyyy" value="${f.fecha_autorizacion}" /></td>
					<td><c:out value="${f.autoridad.nombres}" default="No posee"/> <c:out value="${f.autoridad.apellidos}" default="No posee"/></td>
					<td><fmt:formatNumber value="${f.monto_maximo_autorizado}"/></td>
					<td><c:out value="${f.observaciones}" /></td>
					<td align="center"> <img onclick="elimina(3,'<c:out value="${f.id}" />');" style="cursor:hand" src="<%=request.getContextPath()%>/images/cross.png" width="14" height="13" title="Eliminar"></td>
				</tr>
			</c:forEach>
		</table>
		</div>
		...
		<div  align="center" >
				...
				<div  align="center" >
				    <input class="boton" value="Agregar Nueva Extensión" type="button"  onclick="ir(1);" />
				</div>
