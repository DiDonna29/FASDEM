...
<%
		...
		ArrayList listaPatologias = (ArrayList) request.getAttribute("listaPatologias");
		Patologias pat;
...
%>
	...
	<tiles:put name="titulopagina" content="Administrador de Patologias"
		direct="true" />
...
<div class="sp5"></div>
				...
				<%}else{%>
				<input class="boton" value="Modificar" type="button"  onclick="modificaP('<%=request.getAttribute("id_patologia")%>');" />
				<input class="boton" value="Cancelar" type="button"  onclick="inicio();" />
				<%}%>
				</div>

			<%if(listaPatologias!=null){%>
		...
		<div class="textorange" style="margin-left:5px; padding-bottom: 5px">Listado de Patologias</div>
			...
			<table class="tabla" width="500" cellpadding="0" cellspacing="1" border="0">
				...
				<pg:pager maxPageItems="<%=20%>" maxIndexPages="10" url="adminPatologia.do" isOffset="false" export="offset,currentPageNumber=pageNumber" scope="request">
					<pg:param name="crea" />
					<pg:param name="funcion" />
					<pg:param name="botonera" />		
					<c:forEach items="${listaPatologias}" var="l">
						<pg:item>
		...
		pat = (Patologias) listaPatologias.get(i);
      %>  
				 <tr class="item" bgcolor="white" onclick="ir('<%=pat.getId()%>');">
				
				
					<td><%=pat.getDescripcion()%></td>
					...
					<td align="left" colspan="6">
					 ...
					 <tr>
						<td align="center" class="grayp" style="padding-left: 20px; padding-bottom: 5px;">
							<div class="grayp"></div>
							<strong>No existen Patologias</strong>
						    <div class="grayp"></div>
						</td>
					</tr>		
					 <tr>
...
<script language="JavaScript">
	function ir(acc){
		...
		document.forms[0].action = "<%=request.getContextPath()%>/security/administradores/adminPatologia.do"
