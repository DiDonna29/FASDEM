...
%>
	...
	<tiles:put name="titulopagina" content="Inicio / Pagos / Administrador Hojas de Ruta"
		direct="true" />
	 ...
	 <% if (List!=null && List.size()!=0){%>	
      ...
      %>  		
				 ...
				 <tr class="item" bgcolor="white" >
					...
					<td align="center"><B><%=list.getNumero()%></
					B></td>
		            ...
		            <%if(list.getStatus().getId()==2){ %>
			             <td class="grayplink" align="center"> 
				               <img onclick="cambiar(2,'<%=list.getId()%>','3');" style="cursor:hand" src="<%=request.getContextPath()%>/images/door_out.png" width="14" height="13" title="Abrir">
				         </td>
		            <%}else{%>
		            	<td align="center"><%="---"%></td>
		            <%}%>
