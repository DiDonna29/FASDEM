...
public class RegistroPago extends GenericAction {

	

    static protected Logger log = Logger.getLogger(RegistroPago.class);
       ...
       if (accion==null) // INGRESO A LA PAGINA POR PRIMERA VEZ
		{
           ...
           return mapping.findForward("H1");
			...
			ArrayList list = ExpPreOrdenPago.buscarporCodigoEstatus(request.getParameter("codigo1") + "-" + request.getParameter("codigo2"),anio,Constantes.StatusEditarPagada );
			...
			if(accion.equals("4")){ // ir a la pagina de detalle desde la lista principal 
			...
			if(accion.equals("5")){
			    ...
			    if(request.getParameter("AccSelect1").equals("1")){
			    	...
			    	list = ExpPreOrdenPago.buscarListaProveedorFechaPreOrden(Integer.parseInt(request.getParameter("ProvSelect1")), request.getParameter("dselect1"), request.getParameter("hselect1"),Constantes.StatusONT,anio);
			    }else{
			    	 anio=Integer.parseInt("20"+request.getParameter("CodSelect1").substring(0,2));
			    	 list = ExpPreOrdenPago.buscarporCodigoEstatus(request.getParameter("CodSelect1"),anio,Constantes.StatusEditarPagada);
			    }
			    ...
			    for (int i = 0; i < list.size(); i++) {
			    	...
			    	if (!request.getParameter("fechaPago" + pre.getId()).equals("")) {
			    		    
			    		    ExpPreOrdenPago.cambiarEstatusPagado(pre.getCod_completo(),Constantes.StatusPagado ,Utilidad.StringToDate(request.getParameter("fechaPago" + pre.getId()), "dd/MM/yyyy"), Integer.parseInt("20"+pre.getCod_completo().substring(0,2)));
			    		    incluirTraza(TR_PAGO_CAMBIAR_PREORDEN_PAGADA,
			    		    		pre.getCod_completo(),
									"Cambiar PreOeden a Pagada", usuarioSession(request));
