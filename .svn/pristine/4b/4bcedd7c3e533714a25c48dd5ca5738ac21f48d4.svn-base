...
public class CambiaEstatusPreorden extends GenericAction {
	static protected Logger log = Logger.getLogger(CambiaEstatusPreorden.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (accion == null) // INGRESO A LA PAGINA POR PRIMERA VEZ
		{
			...
			if (accion.equals("5")) {
				...
				ExpPreOrdenPago.administracionEstatusPreOrden(request.getParameter("cod1"), Constantes.StatusEnAdministracion, anio);
				ArrayList list = ExpPreOrdenPago.buscarporCodigo(request.getParameter("cod1"), anio);
				request.setAttribute("lista", list);
				request.setAttribute("primera", "no");
				request.setAttribute("mensaje", "El Estatus de la Pre-Orden de Pago Nro " + request.getParameter("cod1") + " ha sido cambiado satisfactoriamente");
			}
			if (accion.equals("6")) {
