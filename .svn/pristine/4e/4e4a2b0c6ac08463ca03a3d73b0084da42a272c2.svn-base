...
public class EditaPreorden extends GenericAction {
	static protected Logger log = Logger.getLogger(EditaPreorden.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (accion == null) // INGRESO A LA PAGINA POR PRIMERA VEZ
		{
			...
			if (accion.equals("51")) { // / PROCESO DE ELIMINACION DE FACTURA
				...
				incluirTraza(TR_PAGO_ELIMINAR_FACTURA_PREORDEN, request.getParameter("id_fact"), "Id Factura Eliminada de PreOrden", usuarioSession(request));
			...
			if (accion.equals("52")) { // / resgresar a la lista despues de la
			...
			if (accion.equals("3")) { // ir a la pagina de detalle desde la
										// lista principal
				DetallePreOrdenPago det;
				int anio = Integer.parseInt("20" + request.getParameter("cod1").substring(0, 2));
				PreOrdenPago pre = ExpPreOrdenPago.buscarUnicoporCodigo(request.getParameter("cod1"), anio);
				// buscar el detalle segun el tipo, para utilizar los tipos de
				// gasto necesarios
			...
			if (accion.equals("2")) { // imprimir reporte desde la lista de la
										// preorden
			...
			if (accion.equals("8")) { // // BUSQUEDA DE FACTURAS CODIGO DE
										// SINIESTRO
				int anio1 = Integer.parseInt("20" + request.getParameter("codpre").substring(0, 2));
				PreOrdenPago pre = ExpPreOrdenPago.buscarUnicoporCodigo(request.getParameter("codpre"), anio1);
				request.setAttribute("preorden", pre);
			...
			if (accion.equals("9")) { // // BUSQUEDA DE FACTURAS CODIGO DE
										// FACTURA
				int anio1 = Integer.parseInt("20" + request.getParameter("codpre").substring(0, 2));
				PreOrdenPago pre = ExpPreOrdenPago.buscarUnicoporCodigo(request.getParameter("codpre"), anio1);
				request.setAttribute("preorden", pre);
			...
			if (accion.equals("10")) {
				...
				incluirTraza(TR_PAGO_AGREGAR_FACTURA_PREORDEN, request.getParameter("codpre"), "PreOrden Actualizada", usuarioSession(request));
			...
			if (accion.equals("12")) { // / ANULAR PREORDEN
				...
				ExpDetalleFacturaPago.quitarLoteFactPreorden(pre, anio1);
				incluirTraza(TR_PAGO_ANULAR_PREORDEN, request.getParameter("cod1"), "PreOrden Anulada", usuarioSession(request));
				ExpPreOrdenPago.anularPreOrden(pre, Constantes.StatusAnulada, anio1);
