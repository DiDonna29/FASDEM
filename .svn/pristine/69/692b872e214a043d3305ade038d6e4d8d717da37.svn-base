...
%>
			...
			<div style="overflow: visible">
				...
				<input id="estatus" type="hidden" name="estatus" value="<%=Constantes.StatusAutorizado%>">
	 ...
	 <% if (List!=null && List.size()!=0){%>				
      ...
      %>  		
				...
				<tr class="item" bgcolor="white" >
					...
					<td><input type="text" name="fechaOrden<%=list.getId() %>" value="<%=list.getFecha_orden()==null?"":formato.format(list.getFecha_orden())%>" onfocus="javascript:showCalendarSelected2(document.forms[0].fechaOrden<%=list.getId()%>, document.forms[0].fechaOrden<%=list.getId()%>, 'dd/mm/yyyy','es',1);blur();" class="cgp"></td>
					<td><input type="text" name="nro<%=list.getId() %>" value="<%=list.getNro_orden()==null?"":list.getNro_orden()%>" size="10" maxlength="10" class="cgp"></td>
                    ...
                    <td class="grayplink" align="center"> 
	                   <img onclick="javascript:openWinScroll2('<%=request.getContextPath()%>/security/pago/imprimirPreOrden.do?pre=<%=list.getAniomes() + "-" + list.getCodigo_preorden()%>','name',500,500);" style="cursor:hand" src="<%=request.getContextPath()%>/images/printer.png" width="14" height="13" title="Ver">
	                </td>	
	...
	<%}%>    
			...
			<% if (List!=null && List.size()!=0){%>		
				...
				<div  align="center" >
				    <input class="boton" value="Guardar" type="button"  onclick="guarda(5);" />
				</div>
            ...
            <input id="dselect1" type="hidden" name="dselect1" value="<%=request.getAttribute("dselect")%>">
            <input id="hselect1" type="hidden" name="hselect1" value="<%=request.getAttribute("hselect")%>">
            <input id="ProvSelect1" type="hidden" name="ProvSelect1" value="<%=request.getAttribute("ProvSelect")%>">
            <input id="AccSelect1" type="hidden" name="AccSelect1" value="<%=request.getAttribute("AccSelect")%>">
             <input id="CodSelect1" type="hidden" name="CodSelect1" value="<%=request.getAttribute("CodSelect")%>">
