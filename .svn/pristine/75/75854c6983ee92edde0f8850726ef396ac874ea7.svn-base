...
<%
		...
		ArrayList listaOrganos = (ArrayList) request.getAttribute("listaOrganos");
		Organo organo;
...
%>
	...
	<tiles:put name="titulopagina" content="Administrador de Organos"
		direct="true" />
...
<div class="sp5"></div>
				...
				<%}else{%>
				<input class="boton" value="Modificar" type="button"  onclick="modificaP('<%=request.getAttribute("id_organo")%>');" />
			...
			<%if(listaOrganos.size()!=0){%>
			
								<div class="textorange" style="margin-left:5px; padding-bottom: 5px">Listado de Organos</div>
			<div style="overflow: visible">
			<table class="tabla" width="500" cellpadding="0" cellspacing="1" border="0">
				<tr class="tituloCabecera">
					<td>Nombre del Organo</td>
				</tr>
				<pg:pager maxPageItems="<%=20%>" maxIndexPages="10" url="adminOrgano.do" isOffset="false" export="offset,currentPageNumber=pageNumber" scope="request">
					<pg:param name="crea" />
					<pg:param name="funcion" />
					<pg:param name="botonera" />		
					<c:forEach items="${listaOrganos}" var="l">
						<pg:item>
		...
		organo = (Organo) listaOrganos.get(i);
      %>  
				 <tr class="item" bgcolor="white" onclick="ir('<%=organo.getId()%>');">
				
				
					<td><%=organo.getDescripcion()%></td>
					...
					<td align="left" colspan="6">
					 ...
					 <tr>
						<td align="center" class="grayp" style="padding-left: 20px; padding-bottom: 5px;">
							<div class="grayp"></div>
							<strong>No existen Organos</strong>
						    <div class="grayp"></div>
						</td>
					</tr>		
					 <tr>
...
<script language="JavaScript">
	function ir(acc){
		...
		document.forms[0].action = "<%=request.getContextPath()%>/security/administradores/adminOrgano.do"
	...
	function modificaP(acc){
		if(document.getElementById('busca').value ==  ""){
			alert("Favor introducir el nombre del Organo para continuar con la operación.");
			return;
		}
