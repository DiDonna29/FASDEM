...
<td><c:out value="${sini.apellidosBeneficiario}"/></td>								
								...
								<td><c:out value="${sini.lsMontoAmparado}" /><input
									type="hidden" name="id"
									value="<c:out value="${sini.codigo}" />" /></td>
					...
					<pg:index>
						<tr bgcolor="#FFFFFF">
							<td colspan="9" width="100%" align="right" class="textblue">
...
function submit2(argid,anio){
	...
	document.forms[0].action='<%=request.getContextPath()%>/security/cartaAval/reverso/reversar.do';
