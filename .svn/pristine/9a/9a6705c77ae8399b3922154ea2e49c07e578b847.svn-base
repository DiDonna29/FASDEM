...
// 
	...
	var fixedX = 150;		// x position (-1 if to appear below control)
	var fixedY = 150;		// y position (-1 if to appear below control)
	var startAt = 1;		// 0 - sunday ; 1 - monday
	var showWeekNumber = 1;	// 0 - don't show; 1 - show
	var showToday = 1;		// 0 - don't show; 1 - show
	var imgDir = '/fasdem/images/';		// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
	
	var usarListas	= 0;    // 0 - no usa lista, 1 - usa listas.
	var listaDias;
	var listaMeses;
	var listaAnos;
	...
	*/
	...
	if (dom) {
		...
		document.write ('<div onclick="bShow=true" id="calendar" style="z-index:+999;position:absolute;visibility:hidden;"><table width="'+((showWeekNumber==1)?250:220)+'" style="font-family:Arial;font-size:11px;border: 1px solid #000000;" bgcolor="#ffffff"><tr bgcolor="#eef4ff"><td><table width="'+((showWeekNumber==1)?248:218)+'"><tr><td style="padding:2px;font-family:Arial;font-size:11px;"><font color="#000000' + '' /*C9D3E9*/ +'"><b><span id="caption"></span></b></font></td><td align="right"><a href="javascript:hideCalendar()"><img src="'+imgDir+'close.gif" width="15" height="13" border="0" /></a></td></tr></table></td></tr><tr><td style="padding:5px" bgcolor="#ffffff"><span id="content"></span></td></tr>');

		if (showToday == 1) {
			document.write ('<tr bgcolor="#eef4ff"><td style="padding:5px" align="center"><span id="lblToday"></span></td></tr>');
		}
	...
	function init() {
		if (!ns4)
		{
			...
			sHTML1 = '<span id="spanLeft" style="border:1px solid #000000;cursor:pointer" onmouseover="swapImage(\'changeLeft\',\'left2.gif\');this.style.borderColor=\'#000000\';window.status=\''+scrollLeftMessage[language]+'\'" onclick="decMonth()" onmouseout="clearInterval(intervalID1);swapImage(\'changeLeft\',\'left1.gif\');this.style.borderColor=\'#000000\';window.status=\'\'" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartDecMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeLeft" src="'+imgDir+'left1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanRight" style="border:1px solid #000000;cursor:pointer" onmouseover="swapImage(\'changeRight\',\'right2.gif\');this.style.borderColor=\'#000000\';window.status=\''+scrollRightMessage[language]+'\'" onmouseout="clearInterval(intervalID1);swapImage(\'changeRight\',\'right1.gif\');this.style.borderColor=\'#000000\';window.status=\'\'" onclick="incMonth()" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartIncMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeRight" src="'+imgDir+'right1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanMonth" style="border:1px solid #000000;cursor:pointer" onmouseover="swapImage(\'changeMonth\',\'drop2.gif\');this.style.borderColor=\'#000000\';window.status=\''+selectMonthMessage[language]+'\'" onmouseout="swapImage(\'changeMonth\',\'drop1.gif\');this.style.borderColor=\'#000000\';window.status=\'\'" onclick="popUpMonth()"></span>&nbsp;';
			sHTML1 += '<span id="spanYear" style="border:1px solid #000000;cursor:pointer" onmouseover="swapImage(\'changeYear\',\'drop2.gif\');this.style.borderColor=\'#000000\';window.status=\''+selectYearMessage[language]+'\'" onmouseout="swapImage(\'changeYear\',\'drop1.gif\');this.style.borderColor=\'#000000\';window.status=\'\'" onclick="popUpYear()"></span>&nbsp;';
	...
	function closeCalendar() {
		if (usarListas==1){
        	seleccionar(listaDias, listaMeses, listaAnos);
        }
		hideCalendar();
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			document.getElementById('selectMonth').innerHTML = '<table width="70" style="font-family:Arial;font-size:11px;border:1px solid #a0a0a0;" bgcolor="#eef4ff" cellspacing="0" onmouseover="clearTimeout(timeoutID1)" onmouseout="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'popDownMonth()\',100);event.cancelBubble=true">' + sHTML + '</table>';
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			document.getElementById('selectYear').innerHTML = '<table width="44" cellspacing="0" bgcolor="#eef4ff" style="font-family:Arial;font-size:11px;border:1px solid #a0a0a0;" onmouseover="clearTimeout(timeoutID2)" onmouseout="clearTimeout(timeoutID2);timeoutID2=setTimeout(\'popDownYear()\',100)">' + sHTML + '</table>';
	...
	function showCalendarSelected(argDias, argMeses, argAnos, ctl, ctl2, format, lang, past, fx, fy) {
		//usarListas = 0;
		usarListas = 1;
		listaDias = argDias;
		listaMeses = argMeses;
   ...
   function buscarIndice(valor, lista){
	var ret=0;
	var i=0;
	var sw=false;
	while ((i<lista.options.length)&&(!sw)){
	...
	function seleccionar(argListaDias, argListaMeses, argLlistaAnos){
		var indice = 0;

   	}
	function showCalendarSelected2(ctl, ctl2, format, lang, past, fx, fy) {
		//usarListas = 0;
		usarListas = 1;
