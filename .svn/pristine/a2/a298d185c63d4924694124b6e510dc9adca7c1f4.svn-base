...
<tiles:insert page="/jsp/plantilla/plantilla.jsp">
	...
	<tiles:put name="titulopagina" content="Inicio / Liquidaciòn / Bandeja de siniestros por liquidar" direct="true" />
	...
	<tiles:put name="cuerpo" direct="true">
		...
		<input type="hidden" name="tipoTramite" value="${anioBusqueda}"></input>
		<input type="hidden" name="validador" value="1"></input>
		...
		<table class="tabla" width="500" cellpadding="1" cellspacing="1" border="0" bgcolor="yellow">
			...
			<pg:pager maxPageItems="<%=10%>" maxIndexPages="10" url="bandejaPorLiquidar.do" isOffset="false" export="offset,currentPageNumber=pageNumber"
				scope="request">
				<pg:param name="id" />
				<c:forEach items="${lsSiniestros}" var="s">
					<pg:item>
						<tr class="item" bgcolor="white" onclick="javaScript:submit2('<c:out value="${s.id}" />','<c:out value="${s.anioSiniestro}" />')">
							<td class="nblack"><c:out value="${s.codigo}" /></td>
