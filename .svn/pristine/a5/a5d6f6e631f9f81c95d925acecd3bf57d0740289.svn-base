...
<%
		...
		ArrayList listaTratamiento = (ArrayList) request.getAttribute("listaTratamiento");
		Tratamiento tra;
...
%>
	...
	<tiles:put name="titulopagina" content="Administrador de Tratamiento"
		direct="true" />
...
<div class="sp5"></div>
				...
				<%}else{%>
				<input class="boton" value="Modificar" type="button"  onclick="modificaP('<%=request.getAttribute("id_tratamiento")%>');" />
		...
		<div class="textorange" style="margin-left:5px; padding-bottom: 5px">Listado de Tratamientos</div>
			<div style="overflow: visible">
			<table class="tabla" width="500" cellpadding="0" cellspacing="1" border="0">
				<tr class="tituloCabecera">
					<td>Nombre de la Patologia</td>
				</tr>
				<pg:pager maxPageItems="<%=20%>" maxIndexPages="10" url="adminTratamiento.do" isOffset="false" export="offset,currentPageNumber=pageNumber" scope="request">
					<pg:param name="crea" />
					<pg:param name="funcion" />
					<pg:param name="botonera" />		
					<c:forEach items="${listaTratamiento}" var="l">
						<pg:item>
		...
		tra = (Tratamiento) listaTratamiento.get(i);
      %>  
				 <tr class="item" bgcolor="white" onclick="ir('<%=tra.getId()%>');">
					...
					<td align="left" colspan="6">
					 ...
					 <tr>
						<td align="center" class="grayp" style="padding-left: 20px; padding-bottom: 5px;">
							<div class="grayp"></div>
							<strong>No existen Tratamiento</strong>
						    <div class="grayp"></div>
						</td>
					</tr>		
					 <tr>
...
<script language="JavaScript">
	function ir(acc){
		...
		document.forms[0].action = "<%=request.getContextPath()%>/security/administradores/adminTratamiento.do"
