...
<%
		...
		ArrayList listaEspecialidades = (ArrayList) request.getAttribute("listEspecialidades");
...
%>
	...
	<tiles:put name="titulopagina" content="Administrador de Especialidades"
		direct="true" />
...
<div class="sp5"></div>
			...
			<%if(listaEspecialidades.size()!=0){%>
					...
					<div class="textorange" style="margin-left:5px; padding-bottom: 5px">Listado de Especialidades</div>
			<div style="overflow: visible">
			<table class="tabla" width="500" cellpadding="0" cellspacing="1" border="0">
				<tr class="tituloCabecera">
					<td>Nombre de la Especialidad</td>
				</tr>
				<pg:pager maxPageItems="<%=20%>" maxIndexPages="10" url="adminEspecialidad.do" isOffset="false" export="offset,currentPageNumber=pageNumber" scope="request">
					<pg:param name="crea" />
					<pg:param name="funcion" />
					<pg:param name="botonera" />		
					<c:forEach items="${listEspecialidades}" var="l">
						<pg:item>
		...
		especialidad = (Especialidad) listaEspecialidades.get(i);
      %>  
				 <tr class="item" bgcolor="white" onclick="ir('<%=especialidad.getId()%>');">
...
<script language="JavaScript">
	function ir(acc){
		...
		document.forms[0].action = "<%=request.getContextPath()%>/security/administradores/adminEspecialidad.do"
