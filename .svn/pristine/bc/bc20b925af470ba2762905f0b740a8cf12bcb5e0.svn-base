...
%>
	...
	<tiles:put name="titulopagina" content="Inicio / Pagos / Anulación de PreOrdenes"
		direct="true" />
	 ...
	 <% if (List!=null && List.size()!=0){%>				
      ...
      %>  		
				...
				<tr class="item" bgcolor="white" >
					...
					<%if (list.getEstatus().getId()== Constantes.StatusAutorizado || list.getEstatus().getId()== Constantes.StatusEnAdministracion ){ %>
						<td class="grayplink" align="center"> 
		                   <img onclick="irAnulacion(12,'<%=list.getCod_completo()%>');" style="cursor:hand" src="<%=request.getContextPath()%>/images/cross.png" width="14" height="13" title="Anular">
		                </td>
					<%}else{ %>
