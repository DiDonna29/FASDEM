...
<tiles:insert page="/jsp/plantilla/plantilla.jsp">
	...
	<tiles:put name="cuerpo" direct="true">
		...
		<div class="parametro titulo "><a class="textorange" href="<%=request.getContextPath()%>/security/aps/liquidacion/bandejaPorLiquidar.do">Ir
		a la Bandeja de Cartas Avales "Por Liquidar"</a></div>
			...
			<c:forEach items="${facturas}" var="f">
				<tr class="item" bgcolor="white">
					...
					<td><a class="ntextblue" href="#"
						onclick="javascript:openWinScroll2('<%=request.getContextPath()%>/security/cartaAval/liquidacion/cargarDetalleFactura.do?idFact=<c:out value="${f.id}" />&idSini=<c:out value="${siniestro.id}" />','name',500,500);">
					Detalle</a></td>
					<td><a class="nredp"
...
<script language="JavaScript">
	...
	document.forms[0].action='<%=request.getContextPath()%>/security/cartaAval/liquidacion/estatusLiquidado.do'
