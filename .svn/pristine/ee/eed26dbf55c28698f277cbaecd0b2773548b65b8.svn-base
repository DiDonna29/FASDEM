 ...
 * 
 */
public class adminCuentaProveedor extends GenericAction {
     ...
     */
    static protected Logger log = Logger.getLogger(adminCuentaProveedor.class);
       ...
       if (accion==null || "".equals(accion)) // INGRESO A LA PAGINA POR PRIMERA VEZ
		{
    	   ...
    	   return mapping.findForward("input");
			...
			if (request.getParameter("crea")!=null && !"".equals(request.getParameter("crea"))){// BUSCO PROVEEDOR POR RIF
				Clinica prov =(Clinica)ExpClinica.BuscarPorRifCuenta(request.getParameter("crea")).get(0);
				request.setAttribute("proveedor",prov);
				request.setAttribute("listaBanco",ExpBanco.BuscarLista());					
				if(ExpCuenta.ExisteCuentaProveedor(prov.getId())){
			...
			if ("1".equals(request.getParameter("vali"))){
			
			if ("1".equals(accion)){// BUSCO PROVEEDOR POR RIF
				try{
    			request.setAttribute("listProveedores",ExpClinica.BuscarPorRifCuenta(request.getParameter("busca")));
    		...
    		if ("2".equals(accion)){// BUSCO PROVEEDOR POR NOMBRE
    			try{
    			    			request.setAttribute("listProveedores",ExpClinica.BuscarPorNombreCuenta(request.getParameter("busca")));    			
