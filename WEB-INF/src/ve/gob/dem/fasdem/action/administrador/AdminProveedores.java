package ve.gob.dem.fasdem.action.administrador;
...
public class AdminProveedores extends GenericAction{

	public ActionForward execute(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception { 
		ActionMessages am = new ActionMessages();
		Entorno ent1 = new Entorno(Entorno.MOD_EDITA_PROVEEDOR);
		Entorno ent2 = new Entorno(Entorno.MOD_INGRESA_TIPO_GASTO);
		DynaActionForm dForm = (DynaActionForm) form;
		Mapa mapa = new Mapa();
		PerProveedor p = new PerProveedor();
		Proveedor P = null;
		...
		if ("editar".equals(request.getParameter("accion"))) {
			...
			log.info("MAPA: " + mapa);
			log.info("idProveedor: " + request.getParameter("idProveedor"));
			mapa.setId(Integer.parseInt(request.getParameter("idProveedor")));
			p.updateProveedor(mapa);
			...
			try {
				...
				mapa.setIdTipoGasto(Integer.parseInt(request.getParameter("idProveedor")));
				...
				request.setAttribute("idProveedor", P.getId());
