...
public class AdminEspecialidad extends GenericAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		log.info("entre a AdminEspecialidad");	
		ActionMessages am = new ActionMessages();
		Entorno ent1 = new Entorno(Entorno.MOD_EDITA_ESPECIALIDAD);
		Entorno ent2 = new Entorno(Entorno.MOD_INGRESA_ESPECIALIDAD);
		DynaActionForm dForm = (DynaActionForm) form;
		Mapa mapa = new Mapa();
		PerEspecialidad p = new PerEspecialidad();
		Especialidad P = null;
		...
		if ("editar".equals(request.getParameter("accion"))) {
			...
			log.info("MAPA: " + mapa);
			log.info("idEspecialidad: " + request.getParameter("idEspecialidad"));
			mapa.setId(Integer.parseInt(request.getParameter("idEspecialidad")));
			p.updateEspecialidad(mapa);
			...
			dForm.set("idEspecialidad", P.getId());
			...
			try {
				...
				mapa.setIdEspecialidad(Integer.parseInt(request.getParameter("especialidad")));
				mapa.setId(mapa.getIdEspecialidad());
				log.info("MAPA: " + mapa);

				log.info("especialidad "+request.getParameter("especialidad"));

				log.info("Obtiene ID: " + mapa.getIdEspecialidad());
				P = p.search(mapa.getId());
				
				request.setAttribute("idEspecialidad", P.getId());
