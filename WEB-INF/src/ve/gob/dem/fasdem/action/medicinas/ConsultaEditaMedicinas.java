...
public class ConsultaEditaMedicinas extends GenericAction {
	Logger log = Logger.getLogger(ConsultaEditaMedicinas.class);

	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		ActionMessages am = new ActionMessages();
		Entorno ent = new Entorno(Entorno.MOD_MEDICINAS_CONSULTA_EDITA);
		...
		try {
			...
			/***************************************************************************************************/
			if (ent.isCobertura()) {
				int idPoliza = Op.getCobertura().getPoliza().getId();
			...
			log.error("estoy reemplazando ", e1);
