...
public class AnulaPreorden extends GenericAction {


    static protected Logger log = Logger.getLogger(AnulaPreorden.class);
    	   ...
    	   return mapping.findForward("H1");
			...
			if(accion.equals("9")){ //// BUSQUEDA DE FACTURAS CODIGO DE FACTURA
				...
				if(accion.equals("12")){ /// ANULAR PREORDEN
					...
					int hoja_ruta = ExpDetalleFacturaPago.verificaPreorden(pre, anio1);
					log.info("hoja_ruta " + hoja_ruta);
					
					if(hoja_ruta == 0){
				    ExpDetalleFacturaPago.quitarLoteFactPreorden(pre, anio1);
				    
				    incluirTraza(TR_PAGO_ANULAR_PREORDEN,
				    		request.getParameter("cod1") ,
							"PreOrden Anulada", usuarioSession(request));
				    
				    
				    ExpPreOrdenPago.anularPreOrden(pre, Constantes.StatusAnulada, anio1);
					}else{
						request.setAttribute("mensaje", "Alerta, la Pre-Orden pertenece a una Hoja de Ruta");
					}
