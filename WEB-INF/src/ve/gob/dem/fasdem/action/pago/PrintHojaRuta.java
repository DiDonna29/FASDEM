...
public class PrintHojaRuta extends GenericAction {


    static protected Logger log = Logger.getLogger(PrintHojaRuta.class);
...
//							String alias="";
						...
						//ql= "select * from (select numero,CASE WHEN pre_orden_pago.id_proveedor = 44 THEN (select nombres||' '||apellidos from facturas  facturas, siniestro  siniestro "
						   ...
						   if (h.getTipo().getId()==5){
							   rptfilename ="/jasper/HojaDeRutaNomina.jasper"; 
						   }else{
							   rptfilename ="/jasper/HojaDeRuta.jasper"; 
						   }
						 ...
						 //Descompone el resumen
							for (int j = 0; j != RESUMEN.size(); j++) {
								...
								if (r.getTIPO_EMPLEADO() == 1  || r.getTIPO_EMPLEADO() == 9 ) { //EMPLEADO FIJO- -- LIBRE NOMBRAMIENTO
									Emp_Fijo = Emp_Fijo + r.getMONTO_LIQUIDADO();
								}
								...
								if (r.getTIPO_EMPLEADO() == 7) { // / JUBILADOS--
									...
									log.info("jubilados " + r.getMONTO_LIQUIDADO());
								}
