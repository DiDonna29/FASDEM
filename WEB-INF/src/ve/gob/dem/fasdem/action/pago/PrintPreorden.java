...
public class PrintPreorden extends GenericAction {
	static protected Logger log = Logger.getLogger(PrintPreorden.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			log.info("tipo pre orden "+p.getTipo_preorden());
			...
			if (p.getTipo_preorden() == Constantes.TipoPreOrdenFarmacia) {
				if (p.getAplicaTimbre() == 1) {
					rptfilename = "/jasper/FiniquitoDePagoFarmacia.jasper";
				} else {
					rptfilename = "/jasper/FiniquitoDePagoFarmaciaSinTimbre.jasper";
				}
			}
			...
			log.info("aplica ? "+p.getAplicaTimbre());
			...
			for (int j = 0; j != RESUMEN.size(); j++) {
				...
				if (p.getAplicaTimbre() == 1) {
					...
					log.info("monto de timbre  "+ r.getMONTO_TIMBRE());
				} else {
					timbre = 0.00;
				}
				if (p.getTipo_preorden() == Constantes.TipoPreOrdenFarmacia) {
					...
					if (p.getTipo_preorden() == Constantes.TipoPreOrdenReembolsos) {
						...
						retencion = retencion + (iva * p.getRetencion_iva() / 100);
					}
				}
			...
			if (p.getProveedor() != null) {
				if (p.getProveedor().getRif().substring(0, 1).equals("V")) {
					...
					Double isrl_nat = 0.0;
					if (p.getAplicaTimbre() == 1) {
						rptfilename = "/jasper/FiniquitoDePagoNaturales.jasper";
					} else {
						rptfilename = "/jasper/FiniquitoDePagoNaturalesSinTimbre.jasper";
					}
					
					log.info("LIQUIDADO ----------" + liquidado);
			        	log.info("COMPARADOR ----------" + Comparador);
			...
			mapa.put("LIQUIDADO_TOT", Emp_Fijo + Emp_Cont+Obreros+Jubilados+Pensionados+iva);
			...
			mapa.put("sql", sql);
			log.info("sql arriba"+sql);
			log.info("EMP_CONT" + Emp_Cont);
			
			
			path = realPath + rptfilename;
			log.info("prueba "+path);
			// LLENAR EL INFORME
