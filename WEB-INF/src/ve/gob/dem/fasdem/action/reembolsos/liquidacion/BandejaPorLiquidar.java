...
public class BandejaPorLiquidar extends GenericAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		m.setIdTipoTramite(COD_TIPO_TRAMITE_REEMBOLSO);
		m.setIdEstatus(COD_ESTATUS_ANALIZAR);
		m.setAnioSiniestro(getAnioBusqueda(request));
		m.setIdDependencia(usuarioSession(request).getIdDependencia());
		if (request.getParameter("codigo") != null && !"".equals(request.getParameter("codigo"))) {
			m.setCodigo(request.getParameter("codigo"));
		} 
		try {
			log.info("dependenciaaaaaa"+ usuarioSession(request).getIdDependencia());
