            ...
            <c:forEach items="${resultado}" var="sini">
              <pg:item>
                <tr class="item" bgcolor="white"
								onclick="javaScript:submit2('<c:out value="${sini.id}" />','<c:out value="${sini.anioSiniestro}" />')">
                <td><input type="hidden" id="idSini" name="idSini" />
                    <input type="hidden" id="anioSini" name="anioSini" />
                  ...
                  <td><c:out value="${sini.lsTipoSiniestro}" /></td>
                     ...
                     <td><input type="hidden" name="id" value="<c:out value="${sini.codigo}" />"></td>
                </tr>
              </pg:item>
            </c:forEach>
            <pg:index>
              <tr bgcolor="#FFFFFF">
                <td colspan="8" width="100%" align="right" class="textblue"><pg:prev export="pageUrl"> <img height="11"
...
function submit2(argid,anio){
	...
	document.getElementById("anioSini").value=anio;
	
	document.forms[0].action='<%=request.getContextPath()%>/security/cartaAval/detalle.do';
