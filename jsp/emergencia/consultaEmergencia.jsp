...
<tiles:insert page="/jsp/plantilla/plantilla.jsp">
	...
	<tiles:put name="titulopagina" content="Inicio / Emergencia / Consulta de Emergencia"
		direct="true" />
	...
	<input class="cgp" type="text" name="cedula" name="cedula" value="<c:out value='${cedula}'/>"/>
	</div>
	<div class="etiqueta titulo cgp"><bean:message
		key="general.nombres" bundle="etiquetas" /> o <bean:message
		key="general.apellidos" bundle="etiquetas" /></div>
	<div class="parametro titulo ">
	<input class="cgp" type="text" name="nombres" name="nombres" value="<c:out value='${nombres}'/>"/>	
	</div>	
	<div class="etiqueta titulo cgp"><bean:message
		key="general.codigo" bundle="etiquetas" /></div>
	<div class="parametro titulo ">
	<input class="cgp" type="text" name="codigo" name="codigo" value="<c:out value='${codigo}'/>"/>	
	</div>		
					...
					<c:forEach items="${resultado}" var="sini">
						<pg:item>
							<tr class="item" bgcolor="white"
								onclick="javaScript:submit2('<c:out value="${sini.id}" />')">
								<td><input type="hidden" id="idSini"
									name="idSini" /><c:out value="${sini.aniomesCodigo}" />-<c:out
									value="${sini.codigo}" />-<c:out value="${sini.subCodigo}" /></td>
...
<td><c:out value="${sini.apellidosBeneficiario}"/></td>								
								...
								<td><fmt:formatDate pattern="dd/MM/yyyy"
									value="${sini.fechaNotificacion}" /></td>
										<td><fmt:formatDate pattern="dd/MM/yyyy"
									value="${sini.fechaIngreso}" /></td>
										<td><fmt:formatDate pattern="dd/MM/yyyy"
									value="${sini.fechaEgreso}" /></td>
								<td><c:out value="${sini.tipoSiniestro.descripcion}" /></td>
								<td><c:out value="${sini.montoPresupuestado}" /><input
									type="hidden" name="id"
...
function submit2(argid){
	...
	document.forms[0].action='<%=request.getContextPath()%>/security/emergencia/detalle.do';
