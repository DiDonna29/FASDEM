...
<tiles:insert page="/jsp/plantilla/plantilla.jsp">
	...
	<tiles:put name="titulopagina" content="Inicio / Emergencia / Consulta de Emergencia" direct="true" />
	...
	<tiles:put name="cuerpo" direct="true">
		...
		<div class="parametro titulo ">
			<c:out value="${siniestro.aniomesCodigo}"/>-<c:out value="${siniestro.codigo}"/>-<c:out value="${siniestro.subCodigo}"/>
		</div>
		...
		<div class="parametro titulo "><c:if test="${siniestro.citaPostOperatorio!=false}">
			Requiere de Cita PostOperatorio</c:if><c:if test="${siniestro.citaPostOperatorio==false}">
			No Requiere de Cita PostOperatorio</c:if>
		</div>	
		...
		<div class="etiqueta titulo cgp"><bean:message key="general.notatecnicas" bundle="etiquetas" /></div>
		<div class="parametro titulo cgp"><a class="red" href="#"
			onclick="javascript:openWinScroll2('<%=request.getContextPath()%>/security/notatecnica/saveNotaTecnica.do?idSiniestro=<c:out value='${siniestro.id}'/>&anioSiniestro=<c:out	value="${siniestro.anioSiniestro}" />','name',500,500);">Ver Nota Tècnica</a>
		</div>
		<div class="etiqueta titulo cgp"><bean:message key="general.archivos" bundle="etiquetas" /></div>
		<div class="parametro titulo cgp"><a class="red" href="#"
			onclick="javascript:openWinScroll2('<%=request.getContextPath()%>/security/upload/viewFile.do?idSiniestro=<c:out value='${siniestro.id}'/>','name',500,500);">Ver <bean:message key="general.archivos" bundle="etiquetas" /></a>
		</div>
...
<table class="tabla" width="700" cellpadding="2" cellspacing="1"
			border="0"><c:if test="${siniestro.estatus.id==9}">
		...
		<c:if test="${factura.numeroFactura!=null}">
			<table class="tabla" width="600" cellpadding="2" cellspacing="1"
						...
						<tr class="tituloCabecera">
							<td><bean:message key="general.tipoGasto"
								bundle="etiquetas" /></td>
								<td><bean:message key="general.montoPresup"
								bundle="etiquetas" /></td>
								<td><bean:message key="general.montoNego"
								bundle="etiquetas" /></td>
							<td><bean:message key="general.montoAmp"
								bundle="etiquetas" /></td>
							<td><bean:message key="general.montoNoAmp"
								bundle="etiquetas" /></td>

						</tr><c:forEach items="${listDetalle}" var="detalleFacturas">
						<tr class="item" bgcolor="white">
					
							<td><c:out value="${detalleFacturas.tipoGasto.descripcion}" /></td>
							<td align="right"><fmt:formatNumber value="${detalleFacturas.montoPresupuestado}"
								pattern='###0.00' /></td>
							<td align="right"><fmt:formatNumber value="${detalleFacturas.montoNegociado}"
								pattern='###0.00' /></td>
							<td align="right"><fmt:formatNumber value="${detalleFacturas.montoAmparado}"
								pattern='###0.00' /></td>
							<td align="right"><fmt:formatNumber value="${detalleFacturas.montoNoAmparado}"
								pattern='###0.00' /></td>
					...
					<table class="tabla" width="600" cellpadding="2" cellspacing="1"
						...
						</tr><c:forEach items="${listFactura}" var="facturas">
						<tr class="item" bgcolor="white">
